...
#include "DuinoCoinWifi.h"
/*
DuinoCoinWifi::DuinoCoinWifi(HardwareSerial &stream) : serial(stream)
{
...
DuinoCoinWifi::~DuinoCoinWifi(void)
{
  
}

void DuinoCoinWifi::begin(void)
{
...
void DuinoCoinWifi::test(Stream &stream)
{
  stream.print("251 ");
...
// DUCO-S1A hasher
uint32_t DuinoCoinWifi::ducos1a(String lastblockhash, String newblockhash, int difficulty) {
  char buffer[44];
...
uint32_t DuinoCoinWifi::ducos1a(String lastblockhash, String newblockhash, int difficulty) {
  ...
  int ducos1res = 0;
  for (int ducos1res = 0; ducos1res < difficulty * 100 + 1; ducos1res++) {
    String result = SHA1::hash(lastblockhash + ducos1res);
